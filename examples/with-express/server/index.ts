...
app.get("/", async (req, res) => {
  ...
  await script.config(c).default("Writing To Logs works!").record()
...
app.get("/test2", async (req, res) => {
  ...
  await script.config(c).default("test 1").record()
  await script.config(c).default("test 2").record()
  await script.config(c).sid("939").default("test 3").record()
  await script.config(c).sid("939").default("test 4").record()
  await sleep(501)
  res.send("test complete")
})
